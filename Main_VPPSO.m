...
for it = 1:MaxIt
%     figure
w(t) =exp(-(2.5*it/MaxIt)^2.5);
    for i=1:nPop
        
        if rand<0.3
            % Equ. 13
            Velocity(i,:)= abs(Velocity(i,:)).^(rand*w(t))+rand* c1*(Pbest(i,:)-Position(i,:))+rand* c2*(gbest-Position(i,:));
        end

        % Update Velocity
        particle(i).Velocity = w(t)*particle(i).Velocity ...
...
%         Evaluation
    ...
    plot(it,BestCost(it),'*r','MarkerSize',8);axis tight;hold on;grid on;
